...
| 8.5 | **Personal Health Hub — Weekly Review + portable doctor-ready report** | Strong — narrowed 2026-07-19 | Cleanest autonomy (pure string-producing functions, no native/Firebase-write path); novel doctor-prep export. Docked for being a 3-feature bundle and reflective rather than forward-looking. **Narrowed (hygiene 2026-07-19): the weekly-recap half is since covered by the Home hub's council recap (P1); the clean unshipped wedge is the doctor-ready export — picked as Arc A ③.** **Doctor-ready export wedge SHIPPED 2026-07-19 (Arc A ③, PR #216 `doctor-report`): DoctorReport engine + History-panel trigger/panel + Export text helpers. Weekly-recap half remains covered by the Home hub council recap.** |
...
|---|---|---|
| Branch-protection rule on `main` requiring the CI checks | CI is `on: pull_request` only — a direct push bypasses every gate and still deploys; that is the documented flow-vibrate incident path (`.github/workflows/ci.yml` header, `playbooks/stale-cache.md`). A GitHub settings change closes it. **DONE 2026-07-19 (Arc A ②a): protection live — all 7 checks required (incl. `ui-tests`, absent from the older six-check list) + `enforce_admins`; force-push/deletion blocked; repo auto-merge enabled (`gh pr merge --auto --squash`). Direct-push deploy gap closed.** | Low |
...
| Seed `.claude/settings.json` permission allowlist | Read-only loops (`npm test`, `npm run check:*`, `python3 -m http.server`, `git status/diff/log`) each prompt for permission; an allowlist cuts friction for cheap-model sessions. **DONE 2026-07-19 (Arc A ②b, PR #214): 20 read-only entries — exact test-suite forms, parse-only `node --check`, origin-pinned live-site curl, the 3 guard checks, firestore/playwright/kapture reads; code-exec + navigation MCP tools deliberately left prompting.** | Low |
| CLAUDE.md headroom budget | At ~39.4k of the 40k always-loaded limit (2026-06-12). Next relocation candidates when it grows: State Model details → `reference/data-dictionary.md`, per-phase history → `BUILD-HISTORY.md`. Re-run `/trim-context` when >39.5k. **Trimmed 2026-07-19 (Arc A ②c): 48.9k → 38.1k — tooling catalog relocated to `reference/claude-tooling.md`; backlog cells / file-map / Phase-12 / doc-index compressed against their existing offload docs. The >39.5k re-trim trigger stands.** | Low |
...
| 3 | Cloud sync — native CAS + listener parity (`@capacitor-firebase/firestore`) | Medium | Medium | #7 | **Listener parity SHIPPED 2026-07-10** (PR N-1 = #209, merged, cache v166) — native `subscribe()` is real-time via `addCollectionSnapshotListener` (sub-second cross-device sync on iOS instead of the 5-min poll). **Native CAS is IMPOSSIBLE via `@capacitor-firebase/firestore`** — no transaction API in any version through 8.3.0 (verified 2026-07-10; `writeBatch` is write-only atomicity) — permanently out, not deferred: CAS writeback stays defensively skipped on native (web devices converge the cloud; buffer/push setDoc paths unaffected). |
...
| 22 | Life Building — Finances slice (8th synced store, per-month LWW) | High | High | #22 | Slice 1 **LIVE** (PR #198 merged 2026-07-08) — store + council `life_building` synthesizer + hub w/ capture form; council green nightly through 2026-07-19. Close-out = Kyle's July numbers via the capture form → next nightly run → verify `#/life-building` renders real data (hygiene 2026-07-19, Arc A ①). **Arc A ① checked 2026-07-19 evening: `finances` collection still EMPTY; the weekly council run (23:07Z) wrote the honest empty-state record — close-out re-parked awaiting Kyle's numbers.** |
...
**Status (2026-07-09) — Timer MVP built + simulator-validated** (branch `feat/live-activities-timer`). Revived the never-merged 2026-05-22 `feat/live-activities-timer-mvp` draft onto current `main`: custom in-tree `LiveActivityPlugin` + `TempoLiveActivityWidget` Extension + `Platform.liveActivity` bridge + 4 `timer.js` emit points + iOS settings toggle + `tempo://` deep-link (Timer engine only; other engines are the follow-ups). Web verified equivalent; engine suite PASS (1313); `xcodebuild` green; `cap sync` leaves the widget target intact. Runtime validation on an iPhone 17 Pro simulator **caught + fixed a real bug the draft carried** — the plugin registered only via the legacy Obj-C `CAP_PLUGIN` macro, which Capacitor 6.2.1 does not enumerate (see `iOS-BUILD.md` → "Custom (in-tree) Capacitor plugins"); fixed via `CAPBridgedPlugin` conformance + `registerPluginInstance()` in a `CAPBridgeViewController` subclass. Lock-screen activity renders (name + live countdown + progress bar). **Remaining:** PR + Kyle's on-device confirm (incl. the activity-tap deep-link); then reuse the same bridge + Widget target for Stopwatch/Pomodoro/Flow/Interval/Cooking.

**Status (2026-07-10) — Timer MVP SHIPPED + device-validated (8/9 smoke checks) on Kyle's iPhone 17 Pro Max.** PR #201 merged (path-A merge-then-test), then the physical-device smoke drove three fix rounds, all merged + live: **#203** (deep-link route — the widgetURL said `tempo://timers/timer` but the router only knew `countdown`, fell through to Stopwatch; + `applyAppMode` never re-armed timer/pomodoro/interval render loops on switch-in — frozen display; + `loadState` finished-while-away never emitted `endTimer` — orphaned activity; + plugin dict-only lookup lost activities across process relaunch — `findActivity()` registry fallback), **#204** (dead-activity adoption filter `isLive()` — .active/.stale only; robust widget "Done ✓" state — `isStale` OR endsAt-passed, since iOS coalesces stale re-renders; `ProgressView(timerInterval:)` auto-label suppressed — it clamps at the last-emitted window length, the "0:33"/"0:46" mystery; toggle-ON re-arm), **#205** (±3 min adjust now emits the new `endsAt` — lock screen counted to the pre-adjust end otherwise). **Device-confirmed:** lock-screen render, re-lock survival, Dynamic Island compact + long-press expanded, tap→`#/timers/countdown` deep-link, pause freeze / resume single-activity / reset dismiss, finish→"Done ✓" + dismiss-on-open, force-quit persistence with no duplicate on resume.

**OPEN follow-up (backlogged 2026-07-10, Kyle's call — he doesn't use the toggle): settings-toggle OFF→ON re-arm of an in-flight countdown.** Toggle-OFF (`endAll`) works everywhere; the flip-to-ON re-arm **passes on the simulator** (screenshot + ActivityKit log evidence in session log 2026-07-10) but showed nothing on the physical device across three attempts. Diagnosis runbook if resumed: (1) the re-arm is now an all-instances sweep in `js/tempo-nav.js` (`InstanceManager.getTimers()` + `cookingTimers`, running only) — the primary-only gate was the last untested theory alongside device payload staleness; (2) a diagnostic toast variant existed as cache v164 (PR #206, reverted in closeout) — re-add it to see `skipped/ok/reason` on-screen; note Kyle's final v164 run likely raced the CDN (no toast at all = stale payload) and his timer had already overflowed (+0:26) by toggle time, which the sweep correctly skips; (3) remember `⌘R keeps the WebView HTTP cache` — force-quit + >10 min after deploy before trusting a device run; (4) sim driving recipe (AXGroup 1:1 coordinate mapping, `simctl openurl` deep-link, PageDown drawer scroll) is in the session log; Accessibility for osascript got revoked mid-session — re-grant in System Settings before sim-driving again. PR #207 (all-instances sweep with diagnostics) was closed superseded by the closeout PR which landed the sweep without toasts.

**Status (2026-07-10, evening) — Pomodoro + Flow expansion BUILT + sim-validated (PR #211, cache v167).** One shared `TempoTimerAttributes` gains optional `ContentState.label` (updatable phase label: Work 2/4 / Break / Long break / Focus / Recovery) + `attributes.mode` (fixed engine key → widget glyph/tint/deep-link) — supersedes the per-engine-struct sketch: a single activity type keeps `endAll` + registry re-adoption complete, and nil fields render pixel-identical to the device-validated Timer look. **One continuous activity per engine session** (ids `pomodoro`/`flow`): phase transitions update in place (`startTimer` update-routes on an existing id — no end+request race at boundaries), and phase zero-crossings emit nothing (the staleDate contract renders "Done ✓" across the between-phases window — a finished PHASE is not a finished session). Emits live in `js/pomodoro.js`/`js/flow.js` (+28 engine tests, suite 1360); the toggle-ON re-arm sweep covers the singletons; `applyAppMode` now re-arms pomodoro/flow render loops in overflow states (the #203 freeze class — a Done ✓ tap deep-links in exactly there) + 4 UI specs. **Sim evidence (iPhone 17 Pro, local bundled payload):** both deep-links route; red pomodoro island w/ live countdown; zero-cross survival → green DONE; fresh-process reset ends the orphan via registry re-adoption; indigo flow island (1:29:53). **Device-owner checks pending:** expanded-island label chip + lock-screen large-view visuals, widget tap-through, pause badge, hardware toggle re-arm. Remaining engines: Stopwatch (needs a count-up ContentState variant), Interval, Cooking.
