...
| **R9 / SW notification timeouts lost** | audit LOW spike | In-memory `pendingNotifications` Map dies with SW eviction **→ Spiked 2026-07-07: DEFERRED to Phase 4 — fix = persist `{id, fireAt, title, body}` to IDB + re-arm/fire-overdue on SW wake, built WITH the Proving Ground's "notification tap" harness (the only verify path). Web-only; native iOS schedules at the OS level and is unaffected** |
