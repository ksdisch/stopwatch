...
| Native listener parity | `native-sync-parity` — replaces the `'subscribe native parity pending'` throw in `js/sync-firestore.js`'s native branch with a real implementation wrapping `@capacitor-firebase/firestore`'s `addCollectionSnapshotListener` + `removeSnapshotListener`. Mirrors the web branch's deferred-unsubscribe closure shape line-for-line (closure captures `_callbackId` + `_cancelled`; synchronously-returned unsubscribe fn either calls `removeSnapshotListener({ callbackId })` or sets `_cancelled = true` if registration hasn't resolved yet). Plugin event shape `{ snapshots: DocumentSnapshot[] }` converted to web contract `{ docs: [{id, data}], count }` so downstream `js/sync-engine.js` doesn't branch on platform. Both branches of the plugin callback (`event != null` and `error != null`) wrap the caller's callback in try/catch matching the web branch's "must not break the listener" contract; tear-down `removeSnapshotListener` wrapped in `try {} catch (_) {}` matching web's `_realUnsub` failure handling. **iOS latency floor drops ~5min → ~1s** (defensive 5-min poll fallback kept in place per E-3 design; listener is now primary). `runTransaction()` native CAS deferral intentionally kept intact — plugin v6.3.1 exposes `writeBatch` (atomic but not read-conditional) but no `runTransaction`. F19a refuse-writeback on iOS continues to enforce via cloud-side gates in each `js/sync-merge-*.js` module (per-store dispatcher snapshot pre-filter + per-merge-fn schemaVersion check before `setDoc`). +9 tests via kapture port 8765 — 8 native-branch cases in new `tests/sync-firestore.test.js` (~496 LOC) + 1 harness-setup smoke verifying the inline `window.Capacitor` pre-script in `tests/index.html` flips `isNative=true` at module load; baseline 628 → 637 passing. `sw.js` CACHE_NAME bumped v90 → v91. No new persistence keys, no schema bump, no `package.json` dep change, no UI surface touched (Phase 4 ui-wirer skipped — audit's affected-files table is engine + tests + sw.js only). Phase 5 ride-alongs: CLAUDE.md backlog row #2 recast to "listener half shipped, CAS half deferred" framing; SESSION-LOG entry; both audit + brief docs committed for trail. **Listener half of backlog #2 closed; CAS half remains deferred indefinitely pending upstream plugin support.** — shipped 2026-05-19 | #86 |
| F19a (refuse-writeback observability) + F10/F13 (passthrough) | E-3 real-time `onSnapshot` listeners + downlevel-warning toast — second of 2 Stage E reliability follow-ups. New `SyncFirestore.subscribe(path, callback)` wrapper extends the single-Firestore-SDK seam (web-only via lazy-imported `onSnapshot`; native branch throws `kind: 'unknown'` "subscribe native parity pending" mirroring the existing `runTransaction` native deferral; deferred-unsubscribe closure handles SDK-lazy-load race). `js/sync-engine.js` gains 4 new helpers (`_runMergeCycleForStore` per-store dispatch seam + `_subscribeAllStores` / `_scheduleStoreMerge` / `_unsubscribeAllStores` lifecycle); 6 per-collection subscriptions arm inside `startSteadyState` + tear down inside `stopSteadyState`; trailing 1s per-store debounce via `_storeDebouncers` Map; pause-and-unsubscribe on visibility-hidden / network-offline + re-subscribe + one-shot catch-up `_runMergeCycle` on resume; `STEADY_STATE_DEFAULT_MS` bumped 30s → 5min (listeners are now primary; timer is defensive fallback). F19a observability via 3 new `SyncEngine.emit('refuse-writeback', payload)` call sites — dispatcher snapshot pre-filter (`_filterFutureRecordsInSnapshot`) + per-merge-fn cloud-side gate (×6 sync-merge files) + per-record CAS catch (×6 sync-merge files via stashed `_remoteForEmit`); refuse-writeback contract semantics byte-identical. `js/sync-toast.js` extension — `Toast.downlevelWarning(remoteSchemaVersion)` paints the verbatim "Your phone is on a newer version. This device is read-only until you update." copy with 5s auto-dismiss + 200ms fade; once-per-session dedup via module-scope `_downlevelWarningShown` flag; new `'auth-change'` listener resets the dedup flag on sign-out (`user === null` contract); `_resetForTests()` test hook exposed. `js/tempo-nav.js` extends `wireCloudSync` with 3 new `SyncEngine.on(...)` subscribers (`'merge-complete'` / `'listener-connected'` / `'listener-disconnected'`) + closure-scoped `_lastMergeAt` / `_listenerStateByStore` + `_formatSyncStatus()` helper painting the stable text `Last sync: <relative time> · Listeners: <state>` into the existing `#cloud-sync-status` via existing `setStatus(text, false)` (no new DOM, no new CSS per Pick A on TODO #6; `_humanizeRelative` maps sub-second → `'just now'`, sub-minute → `'N sec ago'`, sub-hour → `'N min ago'`, else `'N hr ago'` — reuses `Utils.formatMs` rather than re-implementing). E-2 follow-up symmetry fix at `js/sync-engine.js:1842` closed as one-line ride-along per Pick A on TODO #8 (redundant outer `typeof SyncBuffer !== 'undefined'` guard removed; inner `typeof SyncBuffer.enqueue === 'function'` IS the real feature-detect). +40 tests via kapture fresh-origin port 8765 — 22 new cases in `tests/sync-listeners.test.js` (wrapper + lifecycle + dispatch/debounce + 3-layer refuse-writeback observability) + 9 in `tests/sync-toast.test.js` (downlevelWarning + dedup + sign-out reset + retroactive bufferOverflow coverage) + 9 in `tests/sync-engine.test.js` E-3 block (per-store dispatch + visibility/network lifecycle + symmetry-fix verification) + 2 existing assertions updated (`STEADY_STATE_DEFAULT_MS` 30000 → 300000); baseline 565 → 605 passing. `sw.js` CACHE_NAME bumped v81 → v82, no new ASSETS entries (all E-3 code lives in existing modules). **Phase 4 ui-wirer SMOKE-ONLY — 5th Stage E lineage invocation** (after E-1d-f3, E-1d-f8, E-1e, E-2, E-3) — verified `#cloud-sync-status` text format, downlevel-warning toast paint with verbatim copy, dedup (1 toast for 2 emits), any-disconnected aggregation override, sign-out reset re-arms dedup, neighbor route regression `#/wellness/meds` clean. **Cloud-sync initiative reaches functional completion — Stage E + both reliability follow-ups (E-2 + E-3) shipped; Stage F (F19c per-store manifest registry) remains deferred indefinitely.** Two non-blocking open questions filed for future cleanup: (1) `SyncEngine._resetForTests()` helper mirroring `Toast._resetForTests()` shape would clean up the microtask-flush workaround at sync-engine test #1/#3/#6/#7. (2) Toast "single-slot" semantic has a transient 200ms two-toast window during fade-out — pre-existing E-2 design choice, not new in E-3. — shipped 2026-05-15 | #75 |

**What's pending (sync-strategy items + wire/infra):**

With Stage E (7/7 sub-PRs) + E-2 (offline buffer) + E-3 (real-time listeners + downlevel-warning toast) + `native-sync-parity` (listener half) shipped, the **cloud-sync initiative reaches functional completion across web + iOS for the listener path.** Sub-second cross-device propagation is the new default on both platforms. What remains:
- **Stage F (DEFERRED INDEFINITELY)** — per-store manifest registry (F19c).
- **Native CAS half of backlog #2 (DEFERRED INDEFINITELY)** — `@capacitor-firebase/firestore@^6.3.1` doesn't expose `runTransaction`. F19a refuse-writeback on iOS continues to enforce via cloud-side gates in each `js/sync-merge-*.js` module (status quo since E-1b). Three paths forward: wait for upstream plugin support, contribute the wrapper upstream, or emulate via getDocument+conditional setDocument (rejected — non-atomic breaks F19a).
