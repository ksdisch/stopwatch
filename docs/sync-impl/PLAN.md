...
| — | N-1 native real-time listener parity for `SyncFirestore.subscribe` (backlog #3 — replaces the E-3 "subscribe native parity pending" throw in `js/sync-firestore.js` with a real `addCollectionSnapshotListener` branch: deferred-unsubscribe closure, web-identical `{docs,count}` / `{ok:false,error}` callback shapes, per-doc `hasPendingWrites` M2 echo guard with fail-open on absent metadata, post-cancel suppression + no-leak teardown on late CallbackId resolution, always-async setup-failure callbacks via leading `await Promise.resolve()` per the M3 registry-ordering contract; lazy `_isNative()` refactor — 3 module-scope consts deleted, 6 call sites converted, `runTransaction` byte-preserved honoring the `sync-uploader.test.js` `toString()` naming constraint; **native CAS permanently OUT, not deferred** — no `@capacitor-firebase/firestore` version through 8.3.0 exposes a transaction API (verified 2026-07-10; `writeBatch` is write-only atomicity), merge modules keep defensively skipping the CAS writeback on native; 12 new cases in `tests/sync-listeners.test.js`, engine suite PASS (1332) from baseline 1320; `sw.js` CACHE_NAME bumped v165 → v166) — shipped 2026-07-10 | #209 |

**What's pending (sync-strategy items + wire/infra):**

With Stage E (7/7 sub-PRs) + E-2 (offline buffer) + E-3 (real-time listeners + downlevel-warning toast) + N-1 (native listener parity — iOS now real-time too) shipped, the **cloud-sync initiative reaches functional completion.** Sub-second cross-device propagation is the new default on web AND native. What remains:
