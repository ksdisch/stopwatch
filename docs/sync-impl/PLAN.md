...
| — | Stage E-1b steady-state merge-loop scaffold + per-record CAS wrapper (`SyncEngine.startSteadyState()` / `stopSteadyState()` / `_runMergeCycle()` dispatcher + 4 `js/sync-merge-*.js` IIFE stubs + real `SyncFirestore.runTransaction(fn)` CAS wrapper with new `kind: 'refuse-writeback'`; default-off behind `tempo_sync_steady_state_enabled` strict `=== '1'` flag; web-only CAS — native parity is a documented follow-up; 421/421 tests pass) — shipped 2026-05-13 | (pending PR #) |

**What's pending (sync-strategy items + wire/infra):**

Stage E sub-PRs after E-1b (Option B split per E-1 kickoff): **E-1c** (D-1 reconcile retrofit wiring D-2's `MedsManager.reconcileDoseLog` into `SyncEngine.reconcileImportedBucket()` + replace `sync-merge-meds.js` stub with real meds metadata LWW + doseLog append-merge + F15 ≥2-entry remote-arrival counter; B-4 owns the toast UI subscriber) → **E-1d** (replace `sync-merge-history.js` stub with sessions append-merge dedup + note/tags LWW + phaseLog dedup; lands F3 BFRB stream consolidation + F8 distraction sessionId-keyed migration) → **E-1e** (replace `sync-merge-rest-log.js` + `sync-merge-presets.js` stubs with real merge logic + per-store snapshot F19a refuse-writeback gate + removes dev-flag gate by auto-invoking `startSteadyState()` from `SyncEngine.init()`).
