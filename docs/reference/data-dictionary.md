...
| `live_activities_enabled` | `js/timer.js`, `js/tempo-nav.js` (settings drawer) | `'0'`/`'1'`, **default ON** (`'1'`) when absent | no | no | #4 iOS Live Activities master toggle. Read synchronously by the 4 `timer.js` emit points before firing `Platform.liveActivity.*`; the iOS-only settings-drawer toggle (revealed only when `Platform.liveActivity.isSupported()`) writes it and fires `endAll()` when turned OFF. Per-device like `bfrb_volume`; never synced/exported. |
