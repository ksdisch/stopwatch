...
const PomodoroStats = (() => {
  function getPomodoroSessions() {
    return History.getSessions().filter(s => s.type === 'pomodoro');
  }
  ...
  function getCompletedCyclesThisWeek() {
    const now = new Date();
  ...
  function getTotalWorkMsThisWeek() {
    const now = new Date();
    const weekStart = new Date(now);
  ...
  function getCurrentStreak() {
    const sessions = getPomodoroSessions().filter(s => (s.completedCycles || 0) >= 1);
  ...
  function getDailyMinutesThisWeek() {
    const now = new Date();
    const result = [];
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayMs = getPomodoroSessions()
