...
| 4 | iOS Live Activities — lock screen + Dynamic Island | High | High | #9 | **Timer MVP MERGED + LIVE 2026-07-10** (PR #201, squash `865b735`, cache v160) — custom in-tree Capacitor plugin + Widget Extension + `Platform.liveActivity` bridge + settings toggle + `tempo://` deep-link; runtime validation caught+fixed a Capacitor-6 plugin-registration bug (`CAPBridgedPlugin` + `registerPluginInstance`). Simulator-validated 2026-07-09; on-device smoke in progress (lock-screen render CONFIRMED on iPhone 17 Pro Max; Dynamic Island / tap-deep-link / pause-resume / toggle / force-quit checks pending). Stopwatch/Pomodoro/Flow/Interval/Cooking deferred to follow-ups. |
