...
js/meds.js                      — Medications engine. createMed(id) factory + MedsManager singleton. Interval or time-of-day schedules. Dose logging with optional offset ("took it ~30 min ago"). Prescription supply tracking (opt-in per med): `setSupply(count)` ("New prescription" — stamps `supplyStartCount` + `supplyResetAt`), `clearSupply()` (stop tracking → both null), and derived `getSupplyRemaining()` (= startCount − doses logged on/after the refill, clamped at 0; null when not tracked — which doubles as the "is tracked" flag). Additive nullable fields, no SCHEMA_VERSION bump. D-2 live: `MedsManager.reconcileDoseLog(med, incomingEntries)` (F1 ±15-min cross-device collapse + F16 ±15-min clock-skew clamp + F14 1000-entry cap + F19a future-schema skip) and `MedsManager.onMergeComplete(medId)` (calls `recomputeLastTakenAt` per F4, persists, emits on SyncEngine bus).
js/meds-ui.js                   — Wellness › Meds UI: med cards with live countdown + "since last dose", add/edit form, dose logging, due-time notifications. Supply tracking is opt-in via a "Track prescription supply" checkbox in the add/edit form (+ pills-per-prescription qty); only opted-in cards render the prominent supply badge ("N left" of M, low/empty color states) + "New prescription" inline refill input.
...
- **Engine tests only:** 126 tests cover stopwatch (30), timer (21), pomodoro (25), and meds (50 — incl. 12 for prescription supply tracking) engines — run via `tests/index.html` in a browser. No UI/integration tests yet. Flow and Interval engine tests live on feature branches but haven't been merged to main.
