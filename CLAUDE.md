...
js/synthesis-feed.js            — SynthesisFeed: READ-ONLY consumer of council-written synthesis records (users/{uid}/synthesis/{nodeId}). Mirrors recovery-feed.js: getDoc → per-node localStorage cache → getRecord(nodeId). nodeId encodes '/'→'__'. No write path. See docs/contracts/.
...
- **Phase 12 — Tempo Life-OS Phase 0 (foundations)** — evolved Tempo into the `life-os` trunk *in place* (ADR-0003): Firestore contracts (`docs/contracts/` — synthesis-record + pillar-feed), a local **council** runtime + launchd harness (`council/`, writes synthesis records via the Admin SDK — `recovery_state` pattern), and a read-only synthesis feed (`js/synthesis-feed.js`). Approved plan + ADRs → [`docs/lifeos/`](docs/lifeos/); phased roadmap → [`docs/lifeos/roadmap.md`](docs/lifeos/roadmap.md).
...
- **`#synthesis-debug` readout is temporary (Phase 0):** the settings-drawer debug line in `index.html` (+ its inline init script) exists only to prove the council→Firestore→PWA read path on a phone. Remove it when Phase 1 ships the real Home hub.
