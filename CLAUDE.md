...
- **Phase 12 — Tempo Life-OS (Phase 0 foundations + Phase 1 Home hub)** — evolved Tempo into the `life-os` trunk *in place* (ADR-0003): Firestore contracts (`docs/contracts/` — synthesis-record + pillar-feed), a local **council** runtime + launchd harness (`council/`, writes synthesis records via the Admin SDK — `recovery_state` pattern), and a read-only synthesis feed (`js/synthesis-feed.js`). **Phase 1** shipped the **Home hub** (`js/home-ui.js`): the default-landing dashboard — a Balance bubble map (3 lenses) + per-pillar synthesis cards + "this week's 3 moves", rendered read-only from the council's `home` + 5 domain-pillar records (`SynthesisFeed.refreshAll`/`getAllPillarRecords`). Approved plan + ADRs → [`docs/lifeos/`](docs/lifeos/); phased roadmap → [`docs/lifeos/roadmap.md`](docs/lifeos/roadmap.md).
