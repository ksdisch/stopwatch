...
- `sound_muted`, `sound_profile`, `theme`, `bfrb_volume`, `bfrbs_global`, `wellness_rest_log`, `live_activities_enabled` (per-device toggle for iOS Live Activities; default `'1'` ON when key absent; NOT synced — analogous to `bfrb_volume` / `ambient_volume`)
...
### Phase 11: iOS Live Activities — Timer MVP (2026-05-22)

- **Live Activities for Timer mode (backlog #3/#9 — foundational PR):** Ships the entire JS↔ActivityKit pipeline end-to-end for the `Timer` engine, so subsequent PRs can extend to Stopwatch / Pomodoro / Flow / Interval / Cooking without re-litigating the bridge or the Widget Extension setup. **Six surfaces of change:** (1) new in-tree custom Capacitor plugin (first non-Firebase / non-Capacitor-official plugin in the repo) at `ios/App/App/Plugins/LiveActivity/{LiveActivityPlugin.swift,LiveActivityPlugin.m,TempoTimerAttributes.swift}` — Swift plugin class with 5 `@objc` methods (`isSupported` / `startTimer` / `updateTimer` / `endTimer` / `endAll`), each ActivityKit call wrapped in `if #available(iOS 16.1, *)`-gate, internal `[String: Activity<TempoTimerAttributes>]` dict keyed by `timerId` for resume-from-pause routing to `.update(...)` instead of duplicate `.request(...)`; Obj-C `CAP_PLUGIN(LiveActivityPlugin, "LiveActivity", ...)` macro is what makes `window.Capacitor.Plugins.LiveActivity` populate at runtime; shared `TempoTimerAttributes: ActivityAttributes` struct has target-membership in BOTH the main App target AND the new Widget Extension target. (2) New `TempoLiveActivityWidget` Xcode target at `ios/App/TempoLiveActivityWidget/{TempoLiveActivityWidget.swift,TempoTimerLiveActivity.swift,Info.plist}` — `IPHONEOS_DEPLOYMENT_TARGET = 16.1` (main App stays at 13.0), SwiftUI lock-screen layout (name + PAUSED badge top, large monospaced `Text(timerInterval: Date.now()...endsAt, countsDown: true)` center, thin `ProgressView(timerInterval: startedAt...endsAt)` bottom — all OS-rendered, no per-tick push), Dynamic Island compact leading (timer glyph) + compact trailing (countdown) + expanded (name + larger countdown + progress bar + `.widgetURL(URL(string: "tempo://timers/timer"))` deep-link) + minimal fallback. Static-swap `Text` + `ProgressView(value:)` variants used when `isPaused = true` (Q5 locked decision). (3) `js/platform.js` extension — new `Platform.liveActivity` namespace mirroring `Platform.auth` / `Platform.network` module-pattern verbatim (lazy-init guard, `_liveActivityNativeUnavailableLogged` one-time-warn, normalized `{ supported }` / `{ ok, reason? }` shapes). Web branch is pure no-op. (4) `js/timer.js` — 4 fire-and-forget emit points inside `createTimer()` (`start` / `pause` / `reset` / `checkFinished`-alarm-block), each gated on `localStorage.getItem('live_activities_enabled') !== '0'` and ending with `.catch(() => {})`. Engine state mutations are NOT blocked on the bridge — the emits are pure side-effects. (5) `ios/App/App/Info.plist` — adds `NSSupportsLiveActivities = true` AND a second `<dict>` sibling inside the existing `CFBundleURLTypes` array registering the `tempo://` URL scheme. Existing Google OAuth callback scheme at lines 49–53 is untouched. (6) `index.html` + `js/tempo-nav.js` + `css/styles.css` — new `#live-activities-section` block in `#tempo-settings-drawer` after Cloud Sync, `wireLiveActivities(drawer)` reveals the section only when `Platform.isNative && (await Platform.liveActivity.isSupported()).supported`; `wireAppUrlOpen()` subscribes via `window.Capacitor?.Plugins?.App?.addListener('appUrlOpen', ...)` and maps `tempo://timers/timer` → `window.location.hash = '#/timers/timer'`. Web-side: section stays `hidden`, listener is a total no-op. **Tier:** HIGH per audit — four independent HIGH-tier drivers compound (new Xcode target, first custom Capacitor plugin, `Platform.*` namespace extension, `Info.plist` modification). **xcodebuild verification:** Xcode 26.5 / Swift 6.3.2 → `** BUILD SUCCEEDED **` for both App + TempoLiveActivityWidget targets, zero errors / zero warnings. **Engine tests:** 630 / 630 (no regressions). A `tests/platform.test.js` was created and verified to pass in isolation (6 cases for `Platform.liveActivity` web-branch contract) but ultimately dropped — loading `js/platform.js` into the harness shadows `window.Platform` stubs in 18 pre-existing sync tests. Surfaced as tech debt (refactor 18 sync tests to use property-level Platform stubs instead of wholesale replacement) so future `Platform.X` additions can ship unit tests. **Out of scope (deferred to follow-up PRs):** Stopwatch count-up Live Activity (PR 2); Pomodoro / Flow / Interval / Cooking variants (PR 3+); APNs Push-to-Update; Android equivalent; iOS smoke validation (pending Kyle's device + post-merge `npm run ios:sync`). New persistence key: `live_activities_enabled` (per-device toggle; default `'1'` ON; NOT synced — like `bfrb_volume` / `ambient_volume`). `sw.js` → `v92-live-activities-timer`.
...
| 3 | **iOS Live Activities — running timers on the lock screen + Dynamic Island** | High | High | #9 | **Status: foundational Timer MVP shipped (this PR); subsequent PRs extend to Stopwatch / Pomodoro / Flow / Interval / Cooking.** First PR scaffolded the entire JS↔ActivityKit pipeline: new `TempoLiveActivityWidget` Xcode target (iOS 16.1+, deployment-target-isolated from main App target's 13.0), custom in-tree Capacitor plugin (`ios/App/App/Plugins/LiveActivity/` Swift + Obj-C bridge), shared `TempoTimerAttributes` struct (target-membership in BOTH targets), SwiftUI lock-screen layout (name + PAUSED badge + large `Text(timerInterval:)` countdown + thin `ProgressView(timerInterval:)` progress bar, all OS-rendered — no per-tick push needed) + Dynamic Island compact / expanded / minimal layouts, `Platform.liveActivity` JS bridge with web-side `{ supported: false }` / `{ ok: false, reason: 'web' }` no-op shapes, Timer engine 4 fire-and-forget emit points (`start` / `pause` / `reset` / `_onFinish`) gated on `live_activities_enabled !== '0'`, `NSSupportsLiveActivities` + `tempo://` URL scheme in `Info.plist`, `appUrlOpen` listener in `tempo-nav.js` mapping `tempo://timers/timer` → hash route, settings drawer toggle (default ON). **Follow-up PRs (deferred to subsequent sessions):** Stopwatch count-up Live Activity (PR 2); Pomodoro / Flow / Interval / Cooking variants (PR 3+). **Explicitly out of scope (per original brief):** APNs Push-to-Update (engines are drift-free, local ActivityKit `.update(...)` is sufficient), Android ongoing-notification equivalent. **Unlocked by:** item #1 (Capacitor wrapper already shipped). |
...
`js/platform.js` is the abstraction layer — `Platform.haptic(pattern)`, `Platform.notify(title, opts)`, and `Platform.liveActivity.{isSupported,startTimer,updateTimer,endTimer,endAll}` route to web APIs (`navigator.vibrate` + `Notification`; no-op for Live Activities) on web, and to `@capacitor/haptics` + `@capacitor/local-notifications` + a custom in-tree `LiveActivity` Capacitor plugin on native (Capacitor injects `window.Capacitor.Plugins.*` into the WebView, so no bundler is required). All 23 haptic call sites + 6 immediate-notification call sites now go through `Platform`. `BgNotify.schedule` / `BgNotify.cancel` feature-detect internally and route to `LocalNotifications` on native, so existing call sites in `app.js` / `cooking-ui.js` / etc. don't change. The Live Activities pipeline (introduced in Phase 11) is the first **custom** Capacitor plugin in the repo: in-tree Swift at `ios/App/App/Plugins/LiveActivity/` + Obj-C `CAP_PLUGIN` registration macro + a separate `TempoLiveActivityWidget` Xcode target (iOS 16.1+) for the SwiftUI lock-screen + Dynamic Island layouts. Future custom plugins follow the same pattern.
...
- Live Activities (iOS): `Platform.liveActivity.{isSupported,startTimer,updateTimer,endTimer,endAll}` from `js/platform.js`. Do NOT call `window.Capacitor.Plugins.LiveActivity` directly from engine / UI code — the abstraction layer is responsible for the web no-op shape AND the one-time-warn on plugin-missing.
