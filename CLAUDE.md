...
- **Subagent scope assumes engine-shaped PRs; infrastructure PRs don't fit.** The five specialists in `.claude/agents/` were written assuming the dominant PR shape (engine code + tests + UI). Config-only / infrastructure PRs (S0-1's Firebase setup, B-2's iOS `Info.plist` URL types, future Stage 0 work) need to write files that neither `engine-implementer` (forbidden `docs/*`, `ios/*`) nor `pr-shipper` (restricted doc set) is allowed to touch. **S0-1 worked around this** with a one-off scope expansion in the dispatch brief ("the audit's affected-files table overrides your default forbidden list for THIS PR"). **TODO:** after S0-1 ships, amend `.claude/agents/engine-implementer.md` to make this expansion mechanism explicit — add a clause along the lines of: "If the dispatch brief's `Files in scope` list AND the audit's affected-files table both explicitly enumerate a path outside the default allowed set, treat the brief as authoritative for this PR." This avoids per-PR rule-bending for B-2 and other future infrastructure PRs. Trigger: next time we hit a similar gap (likely B-2's `ios/App/App/Info.plist` for the Firebase reverse-client-id deep link).
