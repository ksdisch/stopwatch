...
| 13 | **Bugfix: Rhythm Timeline dose dots read the deleted legacy `wellness_meds` blob** | Medium | Low | #14 | **Bug (pre-existing, surfaced 2026-06-01 during the Rhythm Insights build #12). Also tracked in "Remaining Tech Debt" below.** `js/rhythm-engine.js` `getDoseEntries()` (~line 119) reads `localStorage['wellness_meds']`, but `js/meds.js` F18 per-record migration (`meds.js:549`) deletes that key after migrating to `meds/{medId}` — so the Rhythm **Timeline** view silently renders **no medication dose events** post-migration. (The Rhythm **Insights** Meds-vs-Sleep panel is unaffected — it reads the live `MedsManager.all()`.) **Fix:** point `getDoseEntries` at `MedsManager.all()` + each med's `getDoseLog()` (mirror the Insights engine's source), and update `tests/rhythm.test.js`, which currently seeds the legacy blob to exercise the dose-timeline path. Small standalone PR — kept out of the Insights PR (#111) because it requires the test-seed change. |
...
- **Rhythm *timeline* dose dots read the deleted legacy `wellness_meds` blob (pre-existing, surfaced 2026-06-01 during the Rhythm Insights build):** `js/rhythm-engine.js` `getDoseEntries()` (~line 119) reads `localStorage['wellness_meds']`, but `js/meds.js` F18 per-record migration (`meds.js:549`) deletes that key after migrating to `meds/{medId}`. So the Rhythm *Timeline* view silently shows no dose events post-migration. The new Rhythm *Insights* Meds-vs-Sleep panel is unaffected — it reads the live `MedsManager.all()`. Fix is a one-liner (point `getDoseEntries` at `MedsManager`), but it also requires updating `tests/rhythm.test.js` (which seeds the legacy blob), so it's deferred to a separate small PR rather than bundled into the Insights work.
