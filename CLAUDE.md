...
- `todoist_pending_ops` (bl-2-todoist; offline queue of pending Todoist write ops [`close`/`reopen`/`create`]. 200-op FIFO cap. Device-local; auto-drains on `online` + `visibilitychange:visible`.)
...
| 5 | **Pomodoro phase revert — "Go back" to previous work or break phase** | Medium | Low | #11 | While in any active Pomodoro phase, a "← Go back" link in the actions row reverts to the previous phase. All time elapsed in the current phase is folded back into the previous phase's accumulated total, so the session records as one continuous block with accurate phase durations. Works both directions (Work→Break and Break→Work). Available indefinitely — no grace-period cutoff. History records the session normally with no special handling required. **Engine (`js/pomodoro.js`):** on every `nextPhase()` call, save a `previousPhaseSnapshot: { phase, cycleIndex, accumulatedMs }` before transitioning. Add `revertPhase()`: compute elapsed time in the current phase (`accumulatedMs + (running ? Date.now() - startedAt : 0)`), restore the snapshot, add elapsed time to the restored `accumulatedMs`. One level of undo only — snapshot is overwritten on each new transition, cleared on `reset()` / `done`. **UI (`js/pomodoro-ui.js`):** "← Go back" link visible in the actions row whenever `previousPhaseSnapshot` exists and `status` is not `'idle'` or `'done'`. **Files:** `js/pomodoro.js`, `js/pomodoro-ui.js`, optionally `css/styles.css`, `sw.js` cache bump. **Risk:** `cycleIndex` must be captured in the snapshot (it governs long-break cadence); restoring it must not double-count the cycle. |
...
| 9 | **Todoist integration follow-up A — Flow user-task list + Todoist integration** | High | Medium | #10-A | Deferred half of backlog row #2. Tempo's current Flow pre-block "checklist" is a hardcoded 5-item ritual (`FLOW_CHECKLIST_ITEMS` in `js/flow-ui.js`), not a user-editable list — so the Pomo V1 PR (#bl-2-todoist, 2026-05-28) couldn't add Todoist imports there directly. This follow-up adds a new user-task list section to the Flow setup view alongside the ritual checklist (new localStorage key `flow_user_tasks` — shape `Array<{ text, todoistId?, done }>`, non-synced, mirrors `pomodoro_saved_tasks` shape post-migration), wires it into the Flow active session UI so completed user tasks decrement a count, and reuses `TodoistUI.openPicker({ onImport })` from PR #bl-2-todoist unchanged (no engine changes needed — `js/todoist.js` already ships everything required). Unlocks: feature-parity with the deferred half of backlog #2. **Files:** `js/flow-ui.js`, `index.html`, `css/styles.css`, sw.js cache bump. Optionally: extract a tiny `js/user-task-list.js` helper if Pomo + Flow start sharing more behavior. |
| 10 | **Todoist integration follow-up B — Pomo inline-rename + Todoist updateTask** | Low | Low | #10-B | Deferred from backlog row #2 V1. Add click-to-edit on Pomodoro saved-task `.text` spans (`<span class="text" contenteditable="false">` → `contenteditable="true"` on click, blur commits, Escape cancels); on blur, persist locally + call `Todoist.updateTask(todoistId, { content })` for items with a `todoistId`. The PR also adds `Todoist.updateTask(id, { content })` to `js/todoist.js` (V1 shipped `closeTask`/`reopenTask`/`createTask` only — no `updateTask` yet) + appends a new offline-queue op kind `'update'` (~50 LoC + 2 smoke steps). **Files:** `js/todoist.js`, `js/pomodoro-ui.js`, `tests/todoist.test.js` (3–4 new cases), sw.js cache bump. |
