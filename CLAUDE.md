...
```
utils → dom-utils → platform → schema → stopwatch → timer → instance-manager → pomodoro → flow → interval → persistence → sync-firebase-config → sync-flag → sync-firestore → sync-buffer → sync-engine → sync-toast → sync-manual-dedupe → sync-merge-meds → sync-merge-history → sync-merge-rest-log → sync-merge-presets → sync-merge-bfrb → sync-merge-distractions → sync-auth → audio → themes → history → export → backup → analog → offset-input → ui → cards-ui → compare-ui → timer-ui → bfrb-recovery → distractions → todoist → todoist-ui → pomodoro-ui → flow-ui → alert-ui → bg-notify → interval-ui → cooking-ui → pomodoro-stats → history-ui → sequence → analytics → focus-ui → sequence-ui → analytics-ui → presets → presets-ui → meds → meds-ui → exercise-ui → mindful-ui → wellness-cooking-ui → recovery-ui → recovery-feed → rhythm-engine → rhythm-insights → rhythm-panel-meds-sleep → rhythm-panel-recovery-trends → rhythm-panel-focus-minutes → rhythm-panel-bfrb-frequency → rhythm-panel-distraction-rollup → rhythm-panel-event-zoom → rhythm-panel-correlations → rhythm-ui → bfrb-events → global-bfrb → tempo-nav → app
```
...
- **Engine tests only:** ~797 `it()` cases across **32 `tests/*.test.js` files** (run via `tests/index.html` in a browser — `curl`-grepping the shell does NOT execute them; the page title self-reports the live PASS/FAIL count). Coverage spans every timing engine (stopwatch/timer/pomodoro/interval), meds, analytics, the Todoist client, schema-stamp helpers, distractions, bfrb-events, and the full cloud-sync stack (engine + uploader + hydrate + 6 per-store merges + auth/buffer/toast/listeners). **Interval (59 cases) IS merged to main** — only the *broad* Flow engine suite remains on a feature branch (`flow.test.js` on main is the narrow F20 set, 7 cases). A small number of recovery-feed cases are known-failing pending a fix (the 2026-05-26 browser-verified note above, 642/642, reflects the then-current suite before later tests landed). Still no UI/integration tests.
...
- **Orphaned readers of the deleted `wellness_meds` blob after the F18 meds migration (partially fixed 2026-06-03):** `js/meds.js`'s F18 per-record migration (`_migrateLegacyBlob`, `meds.js:549`) deletes the legacy `localStorage['wellness_meds']` key after moving each med to its own `meds/{medId}` key — but three consumers still read the old blob. **(1) `js/rhythm-engine.js` `getDoseEntries()` — FIXED** (backlog #13, `fix/rhythm-timeline-meds-source`): now reads `MedsManager.all()` + per-med `getDoseLog()`, so the Rhythm *Timeline* dose dots render again. **(2) `js/analytics.js` `getMedAdherence()` (~line 426) — still broken:** reads `localStorage['wellness_meds']` directly, so the medication-adherence analytics row is silently empty post-migration. Same one-line-ish fix pattern (point at `MedsManager.all()`), but mind the existing "reads localStorage directly so it's safe before app.js inits meds" comment — verify MedsManager is populated at the analytics call site before switching. **(3) `js/export.js` `EXPORT_SETTINGS_KEYS` (~line 110) + `buildBackupData()` (~line 152) — still broken, higher stakes:** lists `wellness_meds` but does a flat `getItem` per key with NO `meds/*` per-record sweep, so **local backups/exports silently omit all medications post-migration** (data-loss-on-backup for users not on cloud sync; synced users are covered by the `meds` Firestore store). Fix needs a per-record `meds/*` enumerate on export + restore (plus tests) — feature-sized, its own PR. Both (2) and (3) are out of scope for the #13 Timeline fix; promote to backlog rows if prioritized.
