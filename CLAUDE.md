...
| 2 | **Cloud sync — native CAS parity for `@capacitor-firebase/firestore`** | Medium | Medium | #7 | **Listener half shipped 2026-05-19** (`native-sync-parity` PR): native `SyncFirestore.subscribe()` now wraps `addCollectionSnapshotListener` + `removeSnapshotListener`, mirroring the web branch's deferred-unsubscribe closure shape line-for-line. iOS cross-device sync propagation drops from ~5min (defensive poll fallback) to ~1s (sub-second listener). **CAS half remains deferred indefinitely** — plugin `@capacitor-firebase/firestore@^6.3.1` exposes `writeBatch` (atomic but not read-conditional) but no `runTransaction`. F19a refuse-writeback on iOS continues to enforce via cloud-side gates in each `js/sync-merge-*.js` module (per-store dispatcher snapshot pre-filter + per-merge-fn schemaVersion check before `setDoc`) — status quo since E-1b shipped 2026-05-13. Three paths to native CAS parity: (a) wait for upstream plugin support, (b) contribute the wrapper upstream, (c) emulate via `getDocument` + conditional `setDocument` (rejected — non-atomic breaks F19a). Requires Xcode + device for any future attempt. |
| 3 | **Split-screen timer comparison** | Medium | High | #2 | Side-by-side two timers. Requires significant layout rework. |
| 4 | **Voice control** | Low | Medium | #3 | Web Speech API SpeechRecognition. Commands: "start", "stop", "lap", "reset". |
| 5 | **Group/team timing** | Low | High | #5 | WebRTC or shared URL with server sync. Major scope expansion — would need a backend. |
