...
js/meds.js                      — Medications engine. createMed(id) factory + MedsManager singleton. Interval or time-of-day schedules. Dose logging with optional offset ("took it ~30 min ago"). Prescription supply tracking: `setSupply(count)` ("New prescription" — stamps `supplyStartCount` + `supplyResetAt`) and derived `getSupplyRemaining()` (= startCount − doses logged on/after the refill, clamped at 0; null when not tracked). Additive nullable fields, no SCHEMA_VERSION bump. D-2 live: `MedsManager.reconcileDoseLog(med, incomingEntries)` (F1 ±15-min cross-device collapse + F16 ±15-min clock-skew clamp + F14 1000-entry cap + F19a future-schema skip) and `MedsManager.onMergeComplete(medId)` (calls `recomputeLastTakenAt` per F4, persists, emits on SyncEngine bus).
js/meds-ui.js                   — Wellness › Meds UI: med cards with live countdown + "since last dose", add/edit form, dose logging, due-time notifications. Prominent prescription-supply badge ("N left" of M, low/empty color states) + "New prescription" inline count input per card.
...
- **Visual verification owed for recent web-session UI changes (no browser automation tool was available):** Two changes shipped from cloud agent sessions that **had no browser automation tool, so the rendering could not be visually confirmed** — flagged here so they get a real-browser pass once an environment with a browser tool (e.g. kapture MCP) is available. (1) **Pomodoro Actions menu always-visible** — the "Actions" link now shows in the idle state too, not just while running; verify the `.pomo-action-links` row wraps cleanly with 5 links (Stats / Settings / Actions / Auto-advance / Saved Tasks) on a narrow phone and that the Actions drawer opens and stays usable while idle. (2) **Meds prescription supply counter** — "New prescription" button + prominent "N left" badge on each med card; verify the badge renders prominently, the inline count input + Enter/Cancel work, the count decrements on each logged dose, and the low (≤5, amber) / empty (0, red) color states fire. Supply engine logic is covered by 11 new `tests/meds.test.js` cases (verified in a Node sandbox), but the canonical browser test run (`tests/index.html`) and the UI rendering still need a real browser.
...
- **Engine tests only:** 125 tests cover stopwatch (30), timer (21), pomodoro (25), and meds (49 — incl. 11 for prescription supply tracking) engines — run via `tests/index.html` in a browser. No UI/integration tests yet. Flow and Interval engine tests live on feature branches but haven't been merged to main.
